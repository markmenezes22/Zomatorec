...
*LLM is not used for: Loading data, Hard filtering by location/budget/rating, Inventing restaurants not in the candidate list.*
...
sequenceDiagram
    ...
    participant Preference Validator
    participant Restaurant Repository
    participant Restaurant Filter
    participant Prompt Builder
    participant Recommendation Service
    participant Groq Client

    User->>Presentation Layer: Enter preferences
    Presentation Layer->>API Controller: POST /recommend
    API Controller->>Preference Validator: validate(preferences)
    Preference Validator-->>API Controller: UserPreferences
    API Controller->>Recommendation Service: 
    Recommendation Service->>Restaurant Repository: get_all() / query
    Restaurant Repository-->>Recommendation Service: restaurants[]
    Recommendation Service->>Restaurant Filter: filter(restaurants, preferences)
    Restaurant Filter-->>Recommendation Service: candidates[]
    Recommendation Service->>Prompt Builder: build(preferences, candidates)
    Prompt Builder-->>Recommendation Service: prompt
    Recommendation Service->>Groq Client: recommend(prompt, candidates)
    Groq Client->>Groq Client: complete(prompt)
    Groq Client-->>Recommendation Service: JSON response
    Recommendation Service->>Recommendation Service: parse & enrich
    Recommendation Service-->>API Controller: RecommendationResponse JSON
    API Controller-->>Presentation Layer: 
    Presentation Layer-->>User: Render ranked cards
```
...
**Minimal production:**
```text
User ──► Streamlit / Static Frontend ──► FastAPI Backend ──► Groq API
                                                │
                                                ▼
                                          Dataset Cache
```
